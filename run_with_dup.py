...
with open("./llvmpasses_with_duplicates.txt", mode="r") as f:
...
ini_cmd1 = 'clang -O0 -emit-llvm -I ./polybench/utilities -I ./polybench/datamining/correlation/correlation -c {path} -o {name}.ll'
ini_cmd2 = 'clang -O0 -emit-llvm -I ./polybench/utilities -I ./polybench/datamining/correlation/correlation -c ./polybench/utilities/polybench.c -o polybench.ll;'
cmd1 = ' -S polybench.ll -o polybench.opt.ll; llc -O0 -filetype=obj polybench.opt.ll -o polybench.o'
cmd2 = ' -S {name}.ll -o {name}.opt.ll; llc -O0 -filetype=obj {name}.opt.ll -o {name}.o'
cmd3 = 'clang -O0 -no-pie -lm *.o -o {name}'
cmd4 = './{name}'
clean_cmd = 'rm -rf *ll *.o *.I *.S {name}'
...
if __name__ == '__main__':
    ...
    if option == 1:
        with open("./llvm_func_passes_with_duplicates.txt", mode="r") as f:
            PASSES = [x.strip() for x in f.readlines()]
    for iter in range(5):
